...
class Trainer:
    ...
    def setup_directories(self):
        ...
        elif self.method == 'random_walk_training':
    ...
    def load_soft_prompts(self,
                         hopping_prompt : nn.Embedding,
        ...
        if load_optimizer:
            ...
            print(f'Loading Optimizer Checkpoint from {soft_prompts_path}')
    ...
    def parse_then_hop(self,
        ...
        for epoch in range(epochs):
            ...
            for batch_idx, batch in enumerate(progress_bar):
                ...
                vram_allocated = torch.cuda.memory_allocated(self.device) / (1024 ** 2)  # Convert to MB
                vram_reserved = torch.cuda.memory_reserved(self.device) / (1024 ** 2)  # Convert to MB
                self.writer.add_scalar('Parse_Then_Hop/Training/VRAM/Allocated', vram_allocated, epoch*len(self.train_dataloader) + batch_idx)
                self.writer.add_scalar('Parse_Then_Hop/Training/VRAM/Reserved', vram_reserved, epoch*len(self.train_dataloader) + batch_idx)

            avg_loss = total_loss / len(self.train_dataloader)
            print(f"Epoch {epoch}, AvgLoss: {avg_loss:.4f}")
            if (self.val_dataloader is not None) and (epoch % self.validation_step == 0):
                if self.evaluate_parse_then_hop(pp_embeddings, hp_embeddings, optimizer, epoch):
                    break
    ...
    def evaluate_parse_then_hop(self,
        ...
        with torch.no_grad():
            ...
            print(f"Epoch {epoch} - Validation - Parsing: AvgLoss: {avg_loss_parsing:.4f} | AvgEM: {avg_em_perc_parsing:.4f} | AvgF1: {avg_f1_perc_parsing:.4f} ")
                  #|||| Hopping: AvgLoss: {avg_loss_hopping:.4f} | AvgEM: {avg_em_perc_hopping:.4f} | AvgF1: {avg_f1_perc_hopping:.4f}")
