...
import sys
class RandomWalkMusiqueDataset(Dataset):
    ...
    """
    def __init__(self, all_kg, validation_dataset, steps, type = 'train'):
        self.kg = all_kg
        self.steps = steps
        
        if type not in ['train', 'dev']:
            raise ValueError(f"Unknown type: {type}. Supported Types are ['train', 'dev']") 
